...
shared (initMsg) actor class FarmFeeReceiver() = this {
    ...
    public shared ({ caller }) func claim(farm : Principal) : async Result.Result<Text, Types.Error> {
        _checkPermission(caller);
        var farmAct = actor (Principal.toText(farm)) : Types.IFarm;
        switch (await farmAct.withdrawRewardFee()) {
